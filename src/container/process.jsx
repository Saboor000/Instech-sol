      ...
      <section className="relative border-b border-white/5 py-24">
        <div className="mx-auto max-w-[1400px] px-6 md:px-10">
          ...
          <div ref={ref} className="relative">
            ...
            <motion.div style={{ scaleX }} className="absolute left-0 right-0 top-8 hidden h-px origin-left bg-accent md:block" />
            <div className="grid gap-12 md:grid-cols-5 md:gap-4">
              {STEPS.map((s, i2) => (
                <Reveal key={s.t} delay={i2 * 0.1}>
                  <div className="relative">
                    ...
                    <div className="font-mono text-xs text-accent">0{i2 + 1} · {s.days}</div>
      ...
      <section className="relative border-y border-white/5 py-20">
        ...
        <div className="group relative overflow-hidden">
          ...
          <div className="marquee-track marquee-pause flex whitespace-nowrap">
            {[...STACK, ...STACK].map((s, i2) => (
              <span key={i2} className="mx-8 flex items-center gap-6 font-display text-5xl font-bold text-white/40 transition-colors hover:text-accent md:text-7xl">
                {s}
          ...
          <div className="grid gap-16 md:grid-cols-[1fr_2fr]">
            ...
            <div className="relative min-h-[280px]">
              <AnimatePresence mode="wait">
                ...
                >
                  <p className="font-display text-2xl leading-snug md:text-4xl">"{QUOTES[i].q}"</p>
                  <div className="mt-8 flex items-center gap-3">
                    <Dot className="!h-2 !w-2" />
                    <span className="font-medium">{QUOTES[i].n}</span>
                    <span className="text-white/50">— {QUOTES[i].r}</span>
                  </div>
                </motion.blockquote>
              </AnimatePresence>
              <div className="mt-6 flex gap-2">
                {QUOTES.map((_, j) => (
                  <button
                    key={j}
                    onClick={() => setI(j)}
                    className={`h-1 rounded-full transition-all ${i === j ? "w-10 bg-accent" : "w-4 bg-white/20"}`}
