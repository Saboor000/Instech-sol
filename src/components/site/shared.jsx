...
import { ArrowUpRight, ArrowRight, Github, Twitter, Linkedin, Instagram, Mail } from "lucide-react";
...
export function PageHero({ eyebrow, title, italic, description }) {
  return (
    <section className="relative overflow-hidden border-b border-white/5 pt-40 pb-24">
      <CodeBackdrop />
      <div className="absolute -top-40 left-1/2 h-[500px] w-[500px] -translate-x-1/2 rounded-full bg-primary/15 blur-[130px]" />
      ...
      <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 py-4 md:px-10">
        ...
        <Link href="/" className="group flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex flex-col leading-none">
            <span className="font-display text-[15px] font-bold tracking-[-0.02em] text-white">
              INSTECH<span className="bg-gradient-to-r from-accent to-primary bg-clip-text text-transparent">SOL</span>
            </span>
            <span className="text-[9px] font-semibold tracking-[0.2em] text-white/40 uppercase mt-0.5">Full-Stack Agency</span>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center">
          <div className={`flex items-center gap-1 rounded-full border px-2 py-2 transition-all duration-500 ${
            scrolled
              ? "bg-white/[0.04] border-white/10 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.4)]"
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <button
            aria-label="Menu"
            onClick={() => setOpen((v) => !v)}
            className="flex h-9 w-9 flex-col items-center justify-center gap-1.5 rounded-full border border-white/15 bg-white/[0.03] backdrop-blur md:hidden transition-colors hover:border-white/30"
          >
            <span className={`block h-px w-4 bg-white transition-all duration-300 ${open ? "translate-y-[3px] rotate-45" : ""}`} />
            <span className={`block h-px w-4 bg-white transition-all duration-300 ${open ? "-translate-y-[3px] -rotate-45" : ""}`} />
          </button>
        </div>
      </div>
...
export function Footer() {
  ...
  return (
    <footer className="relative border-t border-white/5 bg-background pt-24 pb-12 overflow-hidden">
      ...
      <div className="absolute bottom-0 right-0 w-[300px] h-[300px] bg-accent/3 rounded-full blur-[100px] pointer-events-none z-0" />
      ...
      <div className="relative z-10 mx-auto max-w-[1400px] px-6 md:px-10">
        <div className="grid gap-12 md:grid-cols-4 pb-16 border-b border-white/5">
          {/* Brand Col */}
          <div className="md:col-span-2 flex flex-col justify-between">
            <div>
              <Link href="/" className="group inline-flex items-center gap-3">
                <div className="relative flex h-11 w-11 items-center justify-center rounded-xl bg-white border border-white/20 shadow-sm transition-all duration-300 group-hover:shadow-[0_0_16px_rgba(255,255,255,0.25)]">
                  <img
                    src={logoAsset.url}
                ...
                <div className="flex flex-col leading-none">
                  ...
                  <span className="text-[9px] font-semibold tracking-[0.2em] text-white/40 uppercase mt-0.5">
                    Full-Stack Agency
                  </span>
                </div>
              </Link>
              <p className="mt-6 max-w-sm text-sm leading-relaxed text-white/50">
                Crafting powerful digital solutions for modern brands — from Islamabad to the world.
              </p>
            </div>
            
            {/* Availability status indicator */}
            <div className="mt-8 flex items-center gap-3 bg-white/[0.02] border border-white/5 px-4 py-2.5 rounded-full w-fit">
          ...
          <div className="flex flex-col justify-between">
            <div>
              ...
              <a
                href="mailto:hello@instechsol.com"
        ...
        {/* Footer Bottom Row */}
        <div className="mt-8 flex flex-col items-center justify-between gap-6 text-xs text-white/40 md:flex-row">
...
export function CTASection() {
  return (
    <section className="relative overflow-hidden border-t border-white/5 py-32 bg-background">
      <CodeBackdrop />
